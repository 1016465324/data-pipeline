...
import com.wheel.util.DateTimeUtil;
import com.wheel.util.EncodeUtil;
 ...
 * @Date 2020/4/8 10:39
 */
public class JournalParseTask implements Runnable {

    private final Logger logger = LoggerFactory.getLogger(JournalParseTask.class);
    ...
    public JournalParseTask(SyncContext context, JournalRecord journalRecord) {
        this.context = context;
        this.currentJournalRecord = journalRecord;

        topicPartitionNumber = Integer.parseInt(context.getKafkaProps().getProperty("kafka.topic.partition"));

        oggMessage = null;
        globalNode = journalRecord.getGlobalNode();
    }

    /**
     * 解析journal日志文件的记录
     */
    private void parseJournalRecord() {
        ...
        try {
            ...
            if (context.getAllGlobalName().contains(currentJournalRecord.getGlobalNode())) {
                ...
                List<String> namespaces = context.getAllGlobalDbMapNamespace().get(String.format("%s_%s",
                        currentJournalRecord.getGlobalNode(), currentJournalRecord.getDatabaseName()));
                if (namespaces != null) {
                        ...
                        if (allGlobalNameLengthInfo != null) {
                            for (Tuple2<List<StorageSubscriptDefine>, List<ClassPropertyDefine>> globalNameLengthInfo : allGlobalNameLengthInfo) {
                                ...
                                if (compareGlobalNodeInfo(currentJournalRecord.getAllGlobalNode(), globalNameLengthInfo._1)) {
                                    ...
                                    //组装ogg消息格式往topic发送数据
                                    topicName = dataTable.getOutputTopic();
                                    partitionIndex = UtilHelper.wellDistributedHash(
                                            currentJournalRecord.getGlobalNode().hashCode(), topicPartitionNumber);
                                    oggMessage = parseOggMsg();
                                    logger.debug("build ogg message : {}", oggMessage);
                                    break;
                                }
                            }
                        }
...
//            System.out.println(classPropertyDefine.getPropertyName());
    ...
    private String buildQueryParam(List<String> allGlobalRealValue, String globalType) {
        ...
        //参数为class name
        jsonObject.put("table", classInfo._4().getClassName());
    ...
    private JSONArray queryAllGlobal(List<String> allGlobalRealValue, String globalType) {
        ...
        try {
            queryResult = CacheQueryUtil.queryBatchGlobalValue(queryParam);
        } catch (Exception e) {
            logger.error(e.getMessage());
...
//        tableFieldMap.put("rowkey", rowId);
        ...
        //当childsubId有值时,如果值为0,则数据为计数节点,不需要处理,同时主键为0也是计数节点，不需要处理
        return (StringUtils.isNotEmpty(childsubId) && StringUtils.equalsIgnoreCase(childsubId, "0")) ||
                StringUtils.equalsIgnoreCase(rowId, "0");
    }
    ...
    public void run() {
        parseJournalRecord();
        context.getCurrentDealRecordSize().getAndIncrement();
    }

    public JournalRecord getCurrentJournalRecord() {
        return currentJournalRecord;
    }

    public String getOggMessage() {
        return oggMessage;
    }
